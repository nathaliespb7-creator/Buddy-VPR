...
// --- НАСТРОЙКИ CORS (ДОБАВЛЕНО) ---
const ALLOWED_ORIGINS = [
  'https://buddy-vpr-static-natalia.storage.yandexcloud.net',
  'https://buddyvpr.ru',
  'http://localhost:5173'
];

function getCorsHeaders(method: string): Record<string, string> {
  const origin = ALLOWED_ORIGINS.find(o => o === 'https://buddy-vpr-static-natalia.storage.yandexcloud.net') || 
                 ALLOWED_ORIGINS.find(o => o === 'https://buddyvpr.ru') || 
                 ALLOWED_ORIGINS[0];
  ...
  return {
    ...
    'Access-Control-Allow-Credentials': 'true', // ВАЖНО для работы с cookies/авторизацией
...
export async function handler(event: YandexHttpEvent, _context: { requestId?: string }): Promise<YandexHttpResponse> {
  ...
  // Обработка предзапроса OPTIONS (CORS)
  if (method === 'OPTIONS') {
    return {
      statusCode: 204,
  ...
  return new Promise((resolve, reject) => {
    ...
    promise.then(async (response) => {
      // ПРИНУДИТЕЛЬНАЯ ЗАМЕНА ЗАГОЛОВКОВ CORS
      const corsHeaders = getCorsHeaders(method);
      response.headers = {
        ...corsHeaders,
        ...(response.headers || {})
      };
